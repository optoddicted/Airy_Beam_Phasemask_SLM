...
class WinDow:
    ...
    def _display(self):    
        ...
        xa     = x*np.cos(ph)-y*np.sin(ph)
        ya     = x*np.sin(ph)+y*np.cos(ph)        
        
#        phs   = np.angle(np.exp(1j*(xa**3+ya**3)/wp**3))
        phs   = np.mod((xa**3+ya**3)/wp**3,2*np.pi)
        x     = x*np.cos(th)-y*np.sin(th)
        y     = x*np.sin(th)+y*np.cos(th)
        
        
        options = {"Airy_Phase"   : Airy_Phase,
...
def Airy_Phase(phs,x,y,W,Fn,wp):
    Ph = phs
    return Ph
...
root.title('Phase Mask for Airy Beams')

"++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++"
" DECLERATION of VARIABLES "
" --------------------------- Airy Beam Parameter  --------------------------"
wp_in = Tk.DoubleVar(); wp_in.set(100);     " Beam Radius of input Gauss beam "
ph_in = Tk.DoubleVar(); ph_in.set(1);     # Topological Charge 
...
choices = { 'Airy_Phase','Sinusoidal_Grating','Sinusoidal_Gauss','Blazed_Grating'}

tkvar.set('Airy_Phase') # set the default option
